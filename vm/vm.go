...
// runs the VM and starts the fetch-decode-execute cycle
...
func (vm *VM) executeBinaryOperation(op code.Opcode) error {
	...
	case leftType == object.IntegerObj && rightType == object.FloatObj:
		// promote int to float
		return vm.executeBinaryFloatOperation(op, &object.Float{Value: float64(left.(*object.Integer).Value)}, right)
	case leftType == object.FloatObj && rightType == object.IntegerObj:
		// promote int to float
		return vm.executeBinaryFloatOperation(op, left, &object.Float{Value: float64(right.(*object.Integer).Value)})
...
func (vm *VM) executeBinaryFloatOperation(op code.Opcode, left, right object.Object) error {
	...
	case code.OpDiv:
		result = leftValue / rightValue
	case code.OpMod:
		// Floats type doesn't support % operation
		return fmt.Errorf("modulo operator is not supported for floats")
	default:
